      ...
      <PageTransition>
        <section className="px-5 pb-28 pt-6">
          ...
          <div className="mt-6 space-y-5">
            ...
            {members.map((member) => (
              <GarageCard key={member.id} member={member} />
            ))}
          </div>
        </section>
      </PageTransition>
...
function GarageCard({ member }) {
  const gallery = Array.isArray(member.gallery_urls) ? member.gallery_urls : []
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-white/45">{member.full_name}</p>
          </div>
        ...
        {member.bio && (
          <p className="mt-5 text-sm leading-6 text-white/50">{member.bio}</p>
        )}

        <InfoBlock label="Setup" value={member.car_setup || 'Setup nao informado'} />

        {(member.car_specs || member.car_mods) && (
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            {member.car_specs && (
              <InfoBlock label="Specs" value={member.car_specs} compact />
            )}

            {member.car_mods && (
              <InfoBlock label="Mods" value={member.car_mods} compact />
            )}
          </div>
        )}
          ...
          <div className="mt-5 grid grid-cols-3 gap-2">
            {gallery.slice(0, 6).map((imageUrl) => (
              <img
                ...
                src={imageUrl}
                alt={member.car_model}
                className="aspect-square rounded-2xl border border-white/5 object-cover"
              />
            ))}
          </div>
        )}
...
function InfoBlock({ label, value, compact = false }) {
  return (
    <div
      className={
        compact
          ? 'rounded-2xl border border-white/5 bg-black/40 p-4'
          : 'mt-5 rounded-2xl border border-white/5 bg-black/40 p-4'
      }
    >
